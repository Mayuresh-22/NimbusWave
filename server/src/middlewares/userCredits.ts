...
export const UserProjectCreditsMiddleware = createMiddleware<{
  Bindings: Bindings;
  ...
  try {
    ...
    if (
      ...
      getSQLDateTimeNow() <= (userResult.deployment_limit_reset_at as string)
    ) {
      return c.json(
        {
          status: "error",
...
export const UserDeploymentCreditsMiddleware = createMiddleware<{
  Bindings: Bindings;
  Variables: AuthContext;
}>(async (c, next) => {
  try {
    const userResult = await c.env.DB.prepare(
      "SELECT deployment_pm, deployment_limit_reset_at FROM users WHERE id = ?",
    )
      .bind(c.var.user.id)
      .first();
      ...
      return c.json(
        {
          status: "error",
          message:
            "Deployments Per Month limit reached, please wait for the reset",
        },
        402,
      );
    }
