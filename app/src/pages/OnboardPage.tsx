...
export default function OnboardPage() {
  ...
  const handleRegister = async (e: React.FormEvent) => {
    ...
    const registerResponse = await supabase.signUp(registerEmail, registerPassword, {
      name: registerName
    });
    if (!registerResponse) {
      setAlert({
  ...
  useEffect(() => {
    if (location.search === "?confirm_signup=true") {
      ...
      ; (async () => {
        ...
        const userResponse = await user.createUser(userObj?.id as string, userObj?.email as string, {
          ...
          {location.search !== "?complete_signup=true" ? <>
            <div className="bg-gray-800 rounded-lg shadow-lg overflow-hidden">
              <div className="flex border-b border-gray-800">
                <button
                  className={`flex-1 py-4 text-sm font-medium ${activeTab === 'login'
                ...
                <button
                  className={`flex-1 py-4 text-sm font-medium ${activeTab === 'register'
                    ? 'bg-gray-700 text-white'
                    : 'text-gray-400 hover:text-white'
                    }`}
              ...
              <form onSubmit={activeTab === "login" ? handleLogin : handleRegister} className="p-6 space-y-6">
                {activeTab === 'login' ? (
                  <>
